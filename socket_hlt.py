...
class GameMap:
    def __init__(self, size_string, production_string, f, map_string=None):
...
def get_string(f):
    s = f.readline().rstrip('\n')
    #print("received from %s: %s" % (f, s))
    return s


def get_init(f):
    playerID = int(get_string(f))
    m = GameMap(get_string(f), get_string(f), f=f)
    return playerID, m

def send_init(name, f):
    send_string(name, f)
...
def send_frame(moves, f):
    send_string(' '.join(str(move.square.x) + ' ' + str(move.square.y) + ' ' + str(translate_cardinal(move.direction)) for move in moves), f)
